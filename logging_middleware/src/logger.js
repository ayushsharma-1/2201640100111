...
function validateLogParameters(stack, level, packageName, message) {
    // console.log('DEBUG: Validating log parameters:', { stack, level, packageName, messageLength: message?.length }); // For debugging parameter validation
...
async function sendLogToService(stack, level, packageName, message) {
    ...
    // console.log('DEBUG: Sending log to external service:', { endpoint: logEndpoint, tokenLength: accessToken?.length }); // Testing external API integration
    ...
    // console.log('DEBUG: Message truncation applied:', { originalLength: message.length, truncated: message.length > 48, finalMessage: truncatedMessage }); // Testing message truncation logic
